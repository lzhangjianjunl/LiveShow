...
extern zz_flv_script_tag *alloc_zz_flv_script_tag(){
    zz_flv_script_tag *script_tag = zz_alloc(sizeof(zz_flv_script_tag));
    memset(script_tag, 0, sizeof(zz_flv_script_tag));
    
    //初始化
    script_tag->common_tag.script_tag = script_tag;
    script_tag->common_tag.tag_type = zz_flv_tag_type_script;
    
    script_tag->v_codec_id = zz_flv_v_codec_id_H264;
    script_tag->a_codec_id = zz_flv_a_codec_id_AAC;
...
extern void free_zz_flv_script_tag(zz_flv_script_tag ** script_tag){
    zz_free(*script_tag);
    *script_tag = NULL;
}

extern zz_flv_audio_tag *alloc_zz_flv_audio_tag(){
    zz_flv_audio_tag *audio_tag = zz_alloc(sizeof(zz_flv_audio_tag));
    memset(audio_tag, 0, sizeof(zz_flv_audio_tag));
    //初始化
    audio_tag->common_tag.audio_tag = audio_tag;
    audio_tag->common_tag.tag_type = zz_flv_tag_type_audio;
    
    return audio_tag;
}

extern void free_zz_flv_audio_tag(zz_flv_audio_tag ** audio_tag){
    if ((*audio_tag)->config_record_data) {
        free_zz_data(&(*audio_tag)->config_record_data);
    }
    if((*audio_tag)->frame_data){
        free_zz_data(&(*audio_tag)->frame_data);
    }
    zz_free(*audio_tag);
    *audio_tag = NULL;
}
...
extern zz_flv_video_tag *alloc_zz_flv_video_tag(){
    zz_flv_video_tag *video_tag = zz_alloc(sizeof(zz_flv_video_tag));
    memset(video_tag, 0, sizeof(zz_flv_video_tag));
    //初始化
    video_tag->common_tag.video_tag = video_tag;
    video_tag->common_tag.tag_type = zz_flv_tag_type_video;
    
    return video_tag;
}

extern void free_zz_flv_video_tag(zz_flv_video_tag **video_tag){
    if ((*video_tag)->config_record_data) {
        free_zz_data(&(*video_tag)->config_record_data);
    }
    if((*video_tag)->frame_data){
        free_zz_data(&(*video_tag)->frame_data);
    }
    zz_free(*video_tag);
    *video_tag = NULL;
}

static void zz_write_tag_header(zz_data **flv_data, zz_flv_common_tag *common_tag){
...
static void zz_write_audio_tag_body(zz_data **flv_data, zz_flv_audio_tag *audio_tag){
...
static void zz_write_video_tag_body(zz_data **flv_data, zz_flv_video_tag *video_tag){
    ...
    if (video_tag->codec_id == zz_flv_v_codec_id_H264) {
...
static void zz_write_script_tag_body(zz_data **flv_data, zz_flv_script_tag *script_tag){
    //纪录写入了多少字节
    ...
    //打印写入了多少字节
    zzLog("script tag body size = %ld", data_writer.record_size());
    data_writer.end_record_size();
}

static void zz_write_tag_body(zz_data **flv_data, zz_flv_common_tag *common_tag){
    switch (common_tag->tag_type) {
        case zz_flv_tag_type_audio: {
            zz_write_audio_tag_body(flv_data, common_tag->audio_tag);
            break;
        }
        case zz_flv_tag_type_video: {
            zz_write_video_tag_body(flv_data, common_tag->video_tag);
            break;
        }
        case zz_flv_tag_type_script: {
            zz_write_script_tag_body(flv_data, common_tag->script_tag);
            break;
        }
    }
}

static void zz_write_tag_data_size(zz_data **flv_data, zz_flv_common_tag *common_tag){
...
extern void zz_write_flv_tag(zz_data **flv_data, zz_flv_common_tag *common_tag){
    zz_write_tag_header(flv_data, common_tag);
    zz_write_tag_body(flv_data, common_tag);
    zz_write_tag_data_size(flv_data, common_tag);
}
